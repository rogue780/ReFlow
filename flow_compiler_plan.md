# Flow Compiler: Full Build Plan

## Overview

This document is a complete task-level plan for building a reference compiler for the Flow language. The compiler is written in Python, emits C, and is designed to eventually compile itself (bootstrap). Tasks are organized into Epics, each broken into Stories, each broken into discrete implementable Tickets.
...
The runtime is a handwritten C library (`flow_runtime.h` / `flow_runtime.c`) that all generated programs link against.
...
flow/
  ...
  runtime/
    ...
    flow_runtime.c
  tests/
    programs/          ; .flow source files used as test inputs
...
**RT-0-2-1** `[BLOCKER]`
Create `tests/run_tests.py`. It discovers all `.flow` files under `tests/programs/`, compiles each with `flow emit-c`, and diffs the output against the corresponding file in `tests/expected/`. A test passes if the diff is empty. If no expected file exists, the test is marked as "new" and the generated output is written as the new expected file (golden file pattern). Exit code 0 means all tests pass.

**RT-0-2-2**
Create a `tests/programs/hello.flow` with a single `fn:pure add(x: int, y: int): int = x + y` function. Create its expected C output. This is the canary test that runs throughout all subsequent development.
...
- All output identifiers are prefixed with `fl_`
- Dots in module paths become underscores: `math.vector` → `fl_math_vector`
- Type names follow the module: `math.vector.Vec3` → `fl_math_vector_Vec3`
- Methods follow the type: `math.vector.Vec3.dot` → `fl_math_vector_Vec3_dot`
- Top-level functions: `pipeline.orders.run` → `fl_pipeline_orders_run`
...
The runtime is a handwritten C header and implementation file that all generated Flow programs link against. It must be complete enough to support the type system before the emitter is written.
...
**RT-1-1-1** `[BLOCKER]`
In `flow_runtime.h`, define C type aliases for all Flow value types:
...
**RT-1-1-2**
Define overflow-checked arithmetic macros for `fl_int` (and variants). Each macro takes two operands and a result pointer. On overflow, they call `fl_panic_overflow()`. Use GCC/Clang `__builtin_add_overflow`, `__builtin_sub_overflow`, `__builtin_mul_overflow`.
...
Implement for: add, sub, mul for `fl_int`, `fl_int64`, `fl_uint`, `fl_uint64`.

**RT-1-1-3**
Implement `fl_panic_overflow()`, `fl_panic_divzero()`, `fl_panic_oob()` (out of bounds), and `fl_panic(const char* msg)` in `flow_runtime.c`. Each prints a formatted message to stderr and calls `exit(1)`. These are the runtime error handlers for the conditions defined in the spec.
...
typedef struct FL_String {
    fl_int64  refcount;
    fl_int64  len;       /* byte length, not char count */
...
void       fl_string_release(FL_String* s);  /* frees when refcount hits 0 */
FL_String* fl_string_concat(FL_String* a, FL_String* b);
fl_bool    fl_string_eq(FL_String* a, FL_String* b);
fl_int64   fl_string_len(FL_String* s);      /* byte length */
fl_int     fl_string_cmp(FL_String* a, FL_String* b);
```

String literals in generated C use `fl_string_from_cstr("literal")`. The macro `FL_STR("literal")` should expand to a stack-allocated string for constant contexts.

**RT-1-2-2**
Implement `fl_string_new`, `fl_string_from_cstr`, `fl_string_retain`, `fl_string_release`, `fl_string_concat`, `fl_string_eq`, `fl_string_len`, `fl_string_cmp` in `flow_runtime.c`. The refcount starts at 1 on allocation. `fl_string_release` decrements and frees at 0 using `free()`.

**RT-1-2-3**
Implement string-to-numeric conversions: `fl_string_to_int(FL_String* s, fl_int* out) -> fl_bool` (returns false on parse failure) and corresponding functions for `fl_int64`, `fl_float`. Also implement numeric-to-string: `fl_int_to_string(fl_int v) -> FL_String*` and equivalents for other numeric types.
...
Define `FL_NONE` (tag=0) and `FL_SOME(v)` (tag=1, value=v) constructors as macros.

**RT-1-3-2**
Define result types using the same macro pattern. Tag values: `0 = ok`, `1 = err`. The emitter will generate specific result struct typedefs per (T, E) combination encountered in the program. Define the macro and instantiate for `(fl_int, FL_String*)` and `(void*, void*)` as baseline examples.

**RT-1-3-3**
Define `fl_option_unwrap_or(opt, default)` and `fl_result_is_ok(res)`, `fl_result_is_err(res)` as macros for use in generated code.
...
Arrays are immutable. `fl_array_push` returns a new array. OOB access panics.

**RT-1-4-2**
Implement all array functions in `flow_runtime.c`. `fl_array_new` allocates contiguous storage and copies `initial_data` into it if non-null. `fl_array_get_ptr` does bounds checking and calls `fl_panic_oob()` on failure.
...
struct FL_Stream {
    FL_StreamNext next_fn;    /* returns some(element) or none on exhaustion */
    FL_StreamFree free_fn;    /* cleanup when stream is abandoned */
...
Each streaming Flow function compiles to: a frame struct holding all locals, a `next` function that takes the frame and returns `FL_Option_ptr`, and a `free` function that releases frame resources.

**RT-1-5-2**
Implement `fl_stream_new`, `fl_stream_retain`, `fl_stream_release`, `fl_stream_next` in `flow_runtime.c`. When `fl_stream_release` drops the refcount to 0, it calls `free_fn(self)` then `free(self)`.
...
**RT-1-6-1**
Define `FL_Map` as a struct with a void* to an internal hash table implementation. Provide:
...
**RT-1-6-2**
Define `FL_Set` similarly. `fl_set_new`, `fl_set_add`, `fl_set_has`, `fl_set_remove`, `fl_set_len`, `fl_set_retain`, `fl_set_release`. Backed by the same hash table as FL_Map with unit values.
...
FL_Stream* fl_buffer_drain(FL_Buffer* buf);  /* consumes buf */
...
`fl_buffer_collect` pulls from a stream until exhaustion, growing the buffer as needed. If memory allocation fails, calls `fl_panic("BufferOverflowError")`.

**RT-1-7-2**
Implement all buffer functions in `flow_runtime.c`. Use `realloc` for growth with a 2× growth factor. Implement `fl_buffer_drain` by creating a stream whose state is the buffer and whose `next` function walks the index.
...
FL_Stream* fl_stdin_stream(void);      /* stream<byte> from stdin */
FL_Option_ptr fl_read_line(void);     /* reads one line, returns Option<FL_String*> */
```

**RT-1-8-2**
Implement all I/O functions. `fl_stdin_stream` returns a stream whose `next` function reads one byte from stdin and returns it as `some(byte)` or `none` on EOF. `fl_read_line` reads until `\n` or EOF, returning a heap-allocated `FL_String*` wrapped in `some`, or `none` on EOF.
...
The lexer takes a Flow source file as a string and produces a flat list of typed tokens. It is hand-rolled (no external lexer library).
...
**RT-2-1-1** `[BLOCKER]`
In `compiler/lexer.py`, define a `TokenType` enum covering all Flow tokens:
...
**RT-6-4-4**
For `match` on primitive types (int, string, bool), exhaustiveness cannot be fully verified. Emit a compile-time warning if no `_` arm is present. At runtime, an unmatched case calls `fl_panic("match not exhaustive")`.
...
The lowering pass transforms the typed AST into an explicit intermediate form that the C emitter can translate directly. It resolves all Flow abstractions into concrete operations.
...
# LType: maps Flow types to C types
@dataclass
...
@dataclass
class LPtr(LType): inner: 'LType'    # FL_String*, FL_Array*, etc.
@dataclass
...
@dataclass
class LCheckedArith(LExpr):   # calls FL_CHECKED_ADD etc.
    op: str
    left: 'LExpr'
    right: 'LExpr'
    c_type: LType
...
**RT-7-2-1** `[BLOCKER]`
Implement `lower_type(t: Type) -> LType`. Maps Flow types to C types:
...
TArray(T)         → LPtr(LStruct("FL_Array"))
TStream(T)        → LPtr(LStruct("FL_Stream"))
TBuffer(T)        → LPtr(LStruct("FL_Buffer"))
TMap(K, V)        → LPtr(LStruct("FL_Map"))
TSet(T)           → LPtr(LStruct("FL_Set"))
TNone             → LVoid
TOption(int)      → LStruct("FL_Option_int")
...
**RT-7-3-2**
Lower arithmetic: use `LCheckedArith` for all integer arithmetic (maps to `FL_CHECKED_ADD` etc.). Use plain `LBinOp` for float arithmetic.
...
**RT-7-3-4**
Lower `FStringExpr`. Convert to a sequence of `fl_string_concat` calls: `fl_string_concat(fl_string_concat(part1, to_string(expr1)), part2)`. Each non-string part calls its `.to_string()` equivalent C function.
...
**RT-7-3-8**
Lower `CopyExpr` (`@expr`). For immutable heap types: emit `fl_*_retain(ptr); return ptr`. For mutable types: emit a deep copy call. For value types: no-op (value types are already copied).
...
**RT-7-4-1** `[BLOCKER]`
Implement `lower_stmt(stmt: Stmt, types: dict) -> list[LStmt]`. Returns a list because one Flow statement may expand to multiple C statements.

**RT-7-4-2**
Lower `TryStmt`. Implement using `setjmp`/`longjmp` or a simpler approach: each throwable block is wrapped in a function. `retry` is implemented as a loop with a counter. For the bootstrap phase, a straightforward approach: the try block is compiled normally; thrown exceptions unwind via a thread-local exception state struct (`fl_exception_state`). Catching checks the type tag on that struct.

**RT-7-4-3**
Lower `ForStmt` over a stream. Emit a `while` loop that calls `fl_stream_next()` and breaks on `none`. The loop variable is assigned the unwrapped value each iteration.

**RT-7-4-4**
Lower `ForStmt` over an array. Emit a C-style `for` loop with an index variable, using `fl_array_get_ptr` for element access.
...
3. Create a `next` function `(FL_Stream* self) -> FL_Option_ptr` that:
   - Loads the frame from `self->state`
   - Dispatches on the `state` field via `LSwitch`
   - Each state runs code up to the next yield, stores the yielded value, advances state, returns `FL_SOME(value)`
   - The terminal state returns `FL_NONE`
4. Create a `free` function that releases all heap values in the frame and calls `free(frame)`.
5. The original function becomes a factory: allocate the frame, initialize locals, create and return an `FL_Stream*`.

**RT-7-5-2**
Handle `return` inside a streaming function. It must set the state to the terminal value and return `FL_NONE`.
...
**RT-8-2-3**
Implement `emit_ltype(t: LType) -> str`. Returns the C type string for use in declarations: `"int32_t"`, `"double"`, `"bool"`, `"FL_String*"`, `"FL_Stream*"`, struct names, etc.
...
**RT-8-3-8**
Implement temp variable name generation. Each call to `fresh_temp() -> str` returns a new unique name like `_fl_tmp_1`, `_fl_tmp_2`, etc. Reset the counter per function.
...
```c
/* mutable static */
fl_int32 fl_Config_port = 5432;

/* immutable static */
static const fl_int32 fl_Config_max_retries = 3;
```
...
**RT-8-5-1**
Golden file test: compile `tests/programs/hello.flow` (a function `add`), emit C, diff against `tests/expected/hello.c`. The expected C is handwritten to match exactly what the emitter should produce.
...
9. Shell out to `clang -o output tempfile.c flow_runtime.c -I runtime/`
10. Delete temp file
11. Return exit code
...
**RT-9-1-4**
Implement error formatting. On any `FlowError`, print:
```
error[EXXXX]: message
  --> file.flow:12:5
   |
...
**RT-9-2-1**
Implement module discovery. Given a source file `main.flow` with `import math.vector`, the driver looks for `math/vector.flow` relative to the project root. The project root is determined by walking up from the source file until a directory with no parent `module` directory is found, or a `flow.toml` marker file is present.
...
A suite of complete Flow programs that compile and produce correct output.
...
**RT-10-1-1** `[BLOCKER]`
`tests/programs/hello.flow`: print "Hello, World!" to stdout. Compiles, links, runs, and produces correct output.

**RT-10-1-2** `[BLOCKER]`
`tests/programs/fibonacci.flow`: compute the nth Fibonacci number recursively. Tests recursion, integer arithmetic, and basic function calls.

**RT-10-1-3**
`tests/programs/option_test.flow`: tests `option<T>` creation, matching, `??` operator, and auto-lifting.

**RT-10-1-4**
`tests/programs/result_test.flow`: tests `result<T, E>` creation, matching, and `?` propagation through a call chain.

**RT-10-1-5**
`tests/programs/sum_type_test.flow`: defines a `Shape` sum type, implements `area()` via exhaustive `match`.

**RT-10-1-6**
`tests/programs/stream_test.flow`: creates a stream with `yield`, consumes it with `for`, tests `take` and `filter`.

**RT-10-1-7**
`tests/programs/composition_test.flow`: tests `->` chains with fan-out, verifies result matches manual equivalent.

**RT-10-1-8**
`tests/programs/map_test.flow`: creates a `map<string, int>`, inserts, retrieves, checks `has`, iterates keys.

**RT-10-1-9**
`tests/programs/exception_test.flow`: throws a custom exception, catches it, verifies `ex.data` and `ex.original` are accessible.

**RT-10-1-10**
`tests/programs/retry_test.flow`: uses `try`/`retry` with a counter to verify the named function is re-invoked the correct number of times.

**RT-10-1-11**
`tests/programs/pure_test.flow`: a `fn:pure` with a full chain, verified to compile. A non-pure call inside a `fn:pure` is verified to produce a `TypeError`.
...
Rewrite the compiler in Flow and compile it with itself.
...
**RT-11-1-2**
Write the full Flow standard library interfaces needed by the compiler itself:
...
**RT-11-2-1**
Write `compiler/lexer.flow`. Reimplement the lexer in Flow. It must produce identical token sequences to the Python lexer for all test inputs.

**RT-11-2-2**
Write `compiler/ast.flow`. Define all AST nodes as Flow sum types and record types.

**RT-11-2-3**
Write `compiler/parser.flow`. Reimplement the parser in Flow.

**RT-11-2-4**
Write `compiler/resolver.flow`. Reimplement name resolution.

**RT-11-2-5**
Write `compiler/typechecker.flow`. Reimplement the type checker.

**RT-11-2-6**
Write `compiler/lowering.flow`. Reimplement the lowering pass.

**RT-11-2-7**
Write `compiler/emitter.flow`. Reimplement the C emitter.

**RT-11-2-8**
Write `compiler/driver.flow`. Orchestrate all passes and shell out to clang.

**RT-11-2-9**
Write `compiler/main.flow`. Entry point with CLI argument parsing.
...
```
python main.py build compiler/main.flow -o flowc_stage2
```
...
```
./flowc_stage2 build compiler/main.flow -o flowc_stage3
```
If `flowc_stage2` and `flowc_stage3` produce identical binaries (bit-for-bit or with deterministic output), the bootstrap is verified. This is the Makefile `make bootstrap` target.

**RT-11-3-3**
Run the full test suite with `flowc_stage2` as the compiler. All tests that pass with the Python compiler must also pass. Any failure is a bug in the self-hosted compiler.

**RT-11-3-4**
Archive the Python compiler source in `bootstrap/python/` and update `README.md` to reflect that the canonical compiler is now `flowc_stage2`. The Python compiler is no longer maintained but is preserved for historical bootstrapping.
