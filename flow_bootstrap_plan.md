# Flow Bootstrap: Self-Hosted Compiler Plan

## Overview

This document is the complete task-level plan for rewriting the Flow compiler in Flow and bootstrapping it. The Python compiler (Epics 0–10 of `flow_compiler_plan.md`) is the reference implementation. This plan takes it from reference to self-hosted.
...
2. **Rewrite every compiler module in Flow** — same pipeline, same algorithms, same output. The Flow compiler must produce identical C output to the Python compiler on all test programs.
3. **Verify the bootstrap** — compile the Flow compiler with itself and confirm the output is stable (stage 2 = stage 3).

The self-hosted compiler lives in `self_hosted/` and is organized as Flow modules mirroring `compiler/`:

```
self_hosted/
  errors.flow          → error types
  tokens.flow          → Token and TokenType
  ast.flow             → AST node sum types
  types.flow           → Type hierarchy
  lir.flow             → LIR node sum types
  symbols.flow         → Symbol, Scope, ResolvedModule
  mangler.flow         → name mangling
  lexer.flow           → source → tokens
  parser.flow          → tokens → AST
  resolver.flow        → AST → ResolvedModule
  typechecker.flow     → ResolvedModule → TypedModule
  lowering.flow        → TypedModule → LModule
  emitter.flow         → LModule → C string
  driver.flow          → pipeline orchestration
  main.flow            → CLI entry point
```

The compiler is compiled with the Python compiler:
```
python main.py build self_hosted/main.flow -o flowc
```
...
- The Python compiler is the reference throughout. "Same behavior" means normalized-identical C output for the same Flow input (see Parity Testing Strategy below).
- All Flow source in this plan uses the frozen language spec — no new language features are added.
...
The Python compiler uses AST nodes as dictionary keys via Python's identity-based `__hash__`. Flow has no equivalent. Instead, every AST node, Type, and LIR node receives a unique integer `id` field assigned during construction. Side maps use `map<int, T>` keyed by node ID.
...
Python uses `isinstance` dispatch on class hierarchies. Flow uses `match` on sum types. Each node hierarchy becomes a sum type:
...
Python uses `self._field` for mutable state inside compiler passes. Flow has no classes with mutable instance state in the same way. Instead, each pass uses a mutable context record:
...
The emitter builds a C source string. Python uses `list[str]` with `join`. Flow uses `buffer<string>` with `push` and a final join operation. The `string.join` stdlib function joins an array of strings with a separator.

---

## Parity Testing Strategy

The parity test (`verify_parity.py`) compares C output from the Python compiler and the Flow compiler. The comparison uses **normalized parity**, not byte-for-byte identity. Before diffing, both outputs are passed through a normalizer that:

1. Replaces temp variable names (`_fl_tmp_42`, `_fl_e_7`) with sequential counters (`_fl_tmp_0`, `_fl_tmp_1`, ...) per function scope.
...
The Python compiler cannot compile a multi-file Flow program, and it has not been tested with recursive sum types. Both are required before any self-hosted compiler code can be written. These tickets extend the Python compiler — not the Flow code.
...
**RB-0-0-1** `[BLOCKER]`
Implement multi-file compilation in the Python compiler (deferred ticket RT-9-2-1). The self-hosted compiler is 14 `.flow` files that import each other. The Python compiler must:

1. Accept a root module file as input.
2. Parse the root module. For each `import` declaration that is not a stdlib module, resolve it to a `.flow` file relative to the root module's directory.
...
**Definition of done**: `python main.py build self_hosted/main.flow -o flowc` compiles all 14 modules and produces a binary, once those modules have content.

**RB-0-0-2** `[BLOCKER]`
Write tests for multi-file compilation:
- Create `tests/programs/multi/main.flow` that imports `tests/programs/multi/helper.flow`.
- `helper.flow` exports a function. `main.flow` calls it and prints the result.
- Verify `python main.py build tests/programs/multi/main.flow` compiles, links, and runs correctly.
...
Write a test program `tests/programs/recursive_sum.flow`:
```
...
Before writing any compiler code in Flow, the standard library must be extended with string operations, character utilities, file I/O, and process execution. These are implemented as C runtime functions with native Flow wrappers.
...
**RB-1-1-1** `[BLOCKER]`
Add `fl_string_char_at` to the C runtime. Signature:
```c
FL_Option_ptr fl_string_char_at(FL_String* s, fl_int64 idx);
```
...
Create `stdlib/string.flow` with:
```
module string

export fn char_at(s: string, idx: int): char? = native "fl_string_char_at"
```

**RB-1-1-2** `[BLOCKER]`
Add `fl_string_substring` to the C runtime. Signature:
```c
FL_String* fl_string_substring(FL_String* s, fl_int64 start, fl_int64 end);
```
Returns a new string from byte offset `start` (inclusive) to `end` (exclusive). Panics if `start > end` or `end > len`. Add to `stdlib/string.flow`:
```
export fn substring(s: string, start: int, end_idx: int): string = native "fl_string_substring"
```

**RB-1-1-3** `[BLOCKER]`
Add `fl_string_index_of` to the C runtime. Signature:
```c
FL_Option_ptr fl_string_index_of(FL_String* haystack, FL_String* needle);
```
Returns `some(int)` with the byte offset of the first occurrence, or `none` if not found. Add to `stdlib/string.flow`:
```
export fn index_of(s: string, needle: string): int? = native "fl_string_index_of"
```

**RB-1-1-4**
Add `fl_string_contains` to the C runtime. Returns `true` if `needle` appears anywhere in `s`. Add to `stdlib/string.flow`:
```
export fn contains(s: string, needle: string): bool = native "fl_string_contains"
```

**RB-1-1-5**
Add `fl_string_starts_with` and `fl_string_ends_with` to the C runtime. Add to `stdlib/string.flow`:
```
export fn starts_with(s: string, prefix: string): bool = native "fl_string_starts_with"
export fn ends_with(s: string, suffix: string): bool = native "fl_string_ends_with"
```

**RB-1-1-6**
Add `fl_string_split` to the C runtime. Signature:
```c
FL_Array* fl_string_split(FL_String* s, FL_String* sep);
```
Returns an `array<string>`. If `sep` is empty, splits into individual characters. Add to `stdlib/string.flow`:
```
export fn split(s: string, sep: string): array<string> = native "fl_string_split"
```

**RB-1-1-7**
Add `fl_string_trim`, `fl_string_trim_left`, `fl_string_trim_right` to the C runtime. Trims ASCII whitespace (space, tab, newline, carriage return). Add to `stdlib/string.flow`:
```
export fn trim(s: string): string = native "fl_string_trim"
export fn trim_left(s: string): string = native "fl_string_trim_left"
export fn trim_right(s: string): string = native "fl_string_trim_right"
```

**RB-1-1-8**
Add `fl_string_replace` to the C runtime. Replaces all occurrences of `old` with `new_str`. Add to `stdlib/string.flow`:
```
export fn replace(s: string, old: string, new_str: string): string = native "fl_string_replace"
```

**RB-1-1-9**
Add `fl_string_join` to the C runtime. Signature:
```c
FL_String* fl_string_join(FL_Array* parts, FL_String* sep);
```
Joins an array of strings with separator. Add to `stdlib/string.flow`:
```
export fn join(parts: array<string>, sep: string): string = native "fl_string_join"
```

**RB-1-1-10**
Add `fl_string_to_lower` and `fl_string_to_upper` to the C runtime. ASCII-only (sufficient for the compiler). Add to `stdlib/string.flow`:
```
export fn to_lower(s: string): string = native "fl_string_to_lower"
export fn to_upper(s: string): string = native "fl_string_to_upper"
```
...
```
These operate on the `fl_char` type (uint32_t Unicode scalar). For the bootstrap, ASCII classification is sufficient: `is_digit` checks `'0'–'9'`, `is_alpha` checks `'a'–'z'` and `'A'–'Z'` and `'_'`, `is_alphanumeric` is `is_alpha || is_digit`, `is_whitespace` checks space, tab, newline, carriage return.

Create `stdlib/char.flow`:
```
module char

export fn:pure is_digit(c: char): bool = native "fl_char_is_digit"
export fn:pure is_alpha(c: char): bool = native "fl_char_is_alpha"
export fn:pure is_alphanumeric(c: char): bool = native "fl_char_is_alphanumeric"
export fn:pure is_whitespace(c: char): bool = native "fl_char_is_whitespace"
```

**RB-1-2-2**
Add `fl_char_to_int` and `fl_int_to_char` to the C runtime. These are trivial casts between `fl_char` (uint32_t) and `fl_int` (int32_t). Add to `stdlib/char.flow`:
```
export fn:pure to_code(c: char): int = native "fl_char_to_int"
export fn:pure from_code(n: int): char = native "fl_int_to_char"
```

**RB-1-2-3**
Add `fl_char_to_string` to the C runtime. Converts a single char to a one-character string. Add to `stdlib/char.flow`:
```
export fn:pure to_string(c: char): string = native "fl_char_to_string"
```
...
**RB-1-3-1** `[BLOCKER]`
Add `fl_read_file` to the C runtime. Signature:
```c
FL_Option_ptr fl_read_file(FL_String* path);
```
Opens the file at `path`, reads its entire contents into a new `FL_String*`, and returns `some(string)`. If the file cannot be opened or read, returns `none`. Update `stdlib/io.flow`:
```
export fn read_file(path: string): string? = native "fl_read_file"
```

**RB-1-3-2** `[BLOCKER]`
Add `fl_write_file` to the C runtime. Signature:
```c
fl_bool fl_write_file(FL_String* path, FL_String* contents);
```
Writes `contents` to `path`, creating or overwriting the file. Returns `true` on success, `false` on failure. Update `stdlib/io.flow`:
```
export fn write_file(path: string, contents: string): bool = native "fl_write_file"
```
...
**RB-1-4-1** `[BLOCKER]`
Add `fl_run_process` to the C runtime. Signature:
```c
fl_int fl_run_process(FL_String* command, FL_Array* args);
```
Forks and executes the command with the given arguments. Returns the process exit code. Stdout and stderr are inherited (passed through to the calling process). This is used by the driver to shell out to `clang`. Implementation uses `fork`/`exec` on Unix or `posix_spawn`. Update `stdlib/sys.flow`:
```
export fn run_process(command: string, args: array<string>): int = native "fl_run_process"
```

**RB-1-4-2**
Add `fl_run_process_capture` to the C runtime. Same as `fl_run_process` but captures stderr into a string. Signature:
```c
FL_Option_ptr fl_run_process_capture(FL_String* command, FL_Array* args);
```
Returns `some(string)` containing stderr on non-zero exit, `none` on success. Update `stdlib/sys.flow`:
```
export fn run_process_capture(command: string, args: array<string>): string? = native "fl_run_process_capture"
```
...
**RB-1-5-1** `[BLOCKER]`
Add `fl_tmpfile_create` and `fl_tmpfile_remove` to the C runtime. The driver needs to write generated C to a temp file before passing it to clang.
```c
FL_String* fl_tmpfile_create(FL_String* suffix, FL_String* contents);
void       fl_tmpfile_remove(FL_String* path);
```
`fl_tmpfile_create` creates a temp file with the given suffix, writes `contents`, and returns the path. `fl_tmpfile_remove` deletes the file. Update `stdlib/io.flow`:
```
export fn tmpfile_create(suffix: string, contents: string): string = native "fl_tmpfile_create"
export fn tmpfile_remove(path: string): none = native "fl_tmpfile_remove"
```
...
FL_String* fl_path_stem(FL_String* path);      /* "foo/bar.flow" → "bar" */
FL_String* fl_path_parent(FL_String* path);    /* "foo/bar.flow" → "foo" */
FL_String* fl_path_with_suffix(FL_String* path, FL_String* suffix);
FL_String* fl_path_cwd(void);
FL_String* fl_path_resolve(FL_String* path);   /* absolute path */
```
Create `stdlib/path.flow`:
```
module path

export fn join(a: string, b: string): string = native "fl_path_join"
export fn stem(p: string): string = native "fl_path_stem"
export fn parent(p: string): string = native "fl_path_parent"
export fn with_suffix(p: string, suffix: string): string = native "fl_path_with_suffix"
export fn cwd(): string = native "fl_path_cwd"
export fn resolve(p: string): string = native "fl_path_resolve"
```
...
**RB-2-2-1** `[BLOCKER]`
Create the `self_hosted/` directory with empty `.flow` files for each module:
...
Each file has a `module self_hosted.<name>` declaration. The top-level `main.flow` has `module self_hosted.main`.
...
1. Takes a `.flow` source file as input.
2. Runs `python main.py emit-c <file>` to get the Python compiler's C output.
3. Runs `./flowc emit-c <file>` to get the Flow compiler's C output.
4. Diffs the two outputs.
5. Exits 0 if identical, 1 if different, printing the diff.

**RB-2-2-4**
Add Makefile targets:
- `make bootstrap-build` — compiles `self_hosted/main.flow` to `flowc` using the Python compiler.
- `make bootstrap-verify` — runs `verify_parity.py` on all test programs.
- `make bootstrap-stage2` — uses `flowc` to compile `self_hosted/main.flow` to `flowc_stage2`.
- `make bootstrap-stage3` — uses `flowc_stage2` to compile `self_hosted/main.flow` to `flowc_stage3`.
- `make bootstrap-check` — diffs `flowc_stage2` output vs `flowc_stage3` output on all test programs.
...
Define all data types the compiler uses as Flow sum types and records. These are pure data definitions with no logic — the Flow equivalent of `ast_nodes.py`, the Type hierarchy in `typechecker.py`, and the LIR nodes in `lowering.py`.
...
**RB-3-1-1** `[BLOCKER]`
In `self_hosted/errors.flow`, define the error types as a sum type:
...
**RB-3-2-1** `[BLOCKER]`
In `self_hosted/tokens.flow`, define `TokenType` as a sum type with all 118 token variants. Group them logically:
...
**RB-3-3-1** `[BLOCKER]`
In `self_hosted/ast.flow`, define `TypeExpr` as a sum type:
...
**RB-3-4-1** `[BLOCKER]`
In `self_hosted/types.flow`, define the `Type` sum type with all 28 variants:
...
**RB-3-5-1** `[BLOCKER]`
In `self_hosted/lir.flow`, define the LIR type hierarchy:
...
**RB-3-6-1** `[BLOCKER]`
In `self_hosted/symbols.flow`, define resolver data types:
...
Reimplement `compiler/mangler.py` in Flow. Pure string operations. This is the simplest compiler module and a good first test of writing Flow code that matches the Python compiler's output.
...
**RB-4-1-1** `[BLOCKER]`
In `self_hosted/mangler.flow`, implement `mangle(module_path: string, type_name: string?, fn_name: string?): string`. Rules:
- Prefix: `fl_`
- Dots in module path become underscores
- Type name appended after module
- Function/method name appended after type
- Example: `mangle("math.vector", some("Vec3"), some("dot"))` → `"fl_math_vector_Vec3_dot"`

**RB-4-1-2**
Implement `mangle_builtin_type(type_name: string): string`. Maps Flow type names to C typedefs: `"int"` → `"fl_int"`, `"string"` → `"FL_String*"`, `"bool"` → `"fl_bool"`, `"none"` → `"void"`, etc.

**RB-4-1-3**
Implement stream and closure frame manglers:
- `mangle_stream_frame(module_path: string, fn_name: string): string` → `"_fl_frame_<module>_<fn>"`
- `mangle_stream_next(module_path: string, fn_name: string): string` → `"_fl_next_<module>_<fn>"`
- `mangle_stream_free(module_path: string, fn_name: string): string` → `"_fl_free_<module>_<fn>"`
- `mangle_closure_frame(module_path: string, fn_name: string, lambda_id: int): string` → `"_fl_closure_<module>_<fn>_<id>"`

**RB-4-1-4**
Implement C reserved word checking. Maintain a set of C reserved words. If any component of a mangled name (after the `fl_` prefix) collides with a C keyword, the `fl_` prefix prevents collision. Document this invariant.
...
**RB-4-2-1**
Write a test program that calls the Flow mangler and the Python mangler on the same inputs and diffs the output. Test cases: module-only, module+type, module+type+method, top-level function, builtin types, stream frames, closure frames.
...
Reimplement `compiler/lexer.py` in Flow. Character-by-character scanning that produces a `list[Token]` identical to the Python lexer's output.
...
Reimplement `compiler/parser.py` in Flow. Pratt precedence climbing parser that produces an AST Module identical to the Python parser's output. This is the largest single module (~2,400 lines in Python) and will expand to approximately 4,000–5,000 lines in Flow.
...
**RB-6-7-1**
Write parity tests: parse the same source with both the Python and Flow parsers. Since AST structures differ in representation, serialize both to a canonical JSON-like format and diff. Test all declaration types.
...
Reimplement `compiler/resolver.py` in Flow. Scope chain management, symbol table construction, and lambda capture tracking.
...
Reimplement `compiler/typechecker.py` in Flow. This is the most complex module: 1,769 lines in Python, ~28 Type variants, type inference, unification, exhaustiveness checking, purity verification, and stream consumption tracking.
...
**RB-8-7-1**
Implement option auto-lifting. In `check_stmt` for `LetStmt`, if the annotation type is `TOption(T)` and the value type is `T` (not `TOption`), wrap the value in `SomeExpr`. Record the wrapping in the types map so lowering can emit `FL_SOME`.
...
Reimplement `compiler/lowering.py` in Flow. This is the largest module (2,925 lines in Python). It transforms the typed AST into LIR, managing type registries for options, results, tuples, and sum types.
...
- `TString` → `LPtr(LStruct("FL_String"))`
- `TArray(e)` → `LPtr(LStruct("FL_Array"))`
- `TStream(e)` → `LPtr(LStruct("FL_Stream"))`
- `TBuffer(e)` → `LPtr(LStruct("FL_Buffer"))`
- `TMap(k, v)` → `LPtr(LStruct("FL_Map"))`
- `TSet(e)` → `LPtr(LStruct("FL_Set"))`
- `TNone` → `LVoid`
...
**RB-9-1-4**
Implement `fn fresh_temp(ctx: LowererCtx:mut): string`. Returns `"_fl_tmp_N"` with incrementing counter.
...
**RB-9-3-1** `[BLOCKER]`
Implement `fn lower_stmt(ctx: LowererCtx:mut, stmt: Stmt): result<array<LStmt>, CompilerError>`. Pattern-match on all Stmt variants. A single Flow statement may produce multiple LIR statements (e.g., checked arithmetic needs temp variables).

**RB-9-3-2**
Implement lowering for each statement type:
- `LetStmt` → `LVarDecl` with option auto-lifting (wrap in `LCompound` with FL_SOME if needed)
- `AssignStmt` → `LAssign`
- `ReturnStmt` → `LReturn`
...
- `ArrayLit` → `LArrayData` + `LCall("fl_array_new", ...)`
- `FStringExpr` → chain of `fl_string_concat` calls with `fl_*_to_string` for non-string parts
...
Reimplement `compiler/emitter.py` in Flow. Pure formatting of LIR to C source string. This module makes no decisions — it is a deterministic, line-by-line translation.
...
- `fn fresh_temp(ctx: EmitterCtx:mut): string` — `"_fl_e_N"` counter
...
- `LCheckedArith(op, l, r, t)` → emit temp var + `FL_CHECKED_ADD/SUB/MUL` macro call
...
8. Emit `_fl_init_statics()` if any deferred inits exist
9. Emit `main()` wrapper if entry point exists

**RB-10-4-2**
Handle deferred static initialization for string fields:
- If a static is `FL_String*` type with an init, declare as `NULL` and defer init to `_fl_init_statics()`
- `_fl_init_statics()` is called from `main()` before the entry point function
...
**RB-10-5-1** `[BLOCKER]`
Golden file parity test: for each test program in `tests/programs/`, run both the Python and Flow emitters and diff the C output after normalization (see Parity Testing Strategy). This is the critical acceptance test for the entire bootstrap.
...
Reimplement `compiler/driver.py` and `main.py` in Flow. Pipeline orchestration, stdlib module loading, temporary file management, and clang invocation.
...
**RB-11-1-1** `[BLOCKER]`
Implement `fn run_pipeline(source_path: string): result<(string, TypedModule), CompilerError>` in `self_hosted/driver.flow`. Steps:
...
**RB-11-1-3**
Implement `fn load_stdlib_module(module_name: string): result<ModuleScope, CompilerError>`. Locates the stdlib `.flow` file relative to the compiler binary, reads it, lexes, parses, and resolves it.
...
**RB-11-3-1** `[BLOCKER]`
Implement `fn main(): none` in `self_hosted/main.flow`:
...
The CLI accepts:
- `flowc build <file>` — compile to binary
- `flowc emit-c <file>` — emit C only
- `flowc check <file>` — type check only
...
**RB-11-4-1**
Test `compile_source` on `hello.flow` — verify it produces a working binary.

**RB-11-4-2**
Test `emit_only` on `hello.flow` — verify the C output matches the Python compiler's output.
...
- Flow compiler emits C to stdout
...
2. Compile with the Flow compiler → run → capture stdout
...
2. Run through the Flow compiler → capture error
...
**RB-12-3-2**
Create a stress test: a Flow program with 100+ functions, deeply nested match expressions, large sum types. Verify the Flow compiler handles it without stack overflow or performance degradation.
...
- `flowc build <file>` — produces a binary
- `flowc emit-c <file>` — emits C source to stdout (critical for debugging)
- `flowc check <file>` — type checks only (critical for narrowing bugs to a specific pass)
...
1. Run `flowc emit-c <failing_file>` and `python main.py emit-c <failing_file>` — diff the C output to find structural divergence.
2. Run `flowc check <failing_file>` to verify the type checker agrees. If it disagrees, the bug is in the type checker.
...
The final milestone. Compile the Flow compiler with itself and verify the output is stable.

---

## Story 13-1: Stage 2 — Python Compiles Flow Compiler

**RB-13-1-1** `[BLOCKER]`
Compile the self-hosted compiler using the Python compiler:
```
python main.py build self_hosted/main.flow -o flowc_stage2
```

This produces the first self-hosted binary. It must:
- Compile without errors
- Pass `flowc_stage2 check` on all test programs
...
**RB-13-3-1**
Run the complete test suite (`make test`) using `flowc_stage2` as the compiler instead of the Python compiler. All tests must pass:
...
**RB-13-3-2**
Run the E2E test suite using `flowc_stage3` as the compiler. All tests must pass identically to stage 2.
...
Update `README.md` to reflect that the canonical compiler is now `flowc` (the self-hosted binary). The Python compiler is preserved for historical bootstrapping but is no longer maintained.
...
**RB-13-4-3**
Update `CLAUDE.md` to reflect the new project state: the compiler is self-hosted, the Python source is archived, and all future development happens in Flow.
...
EPIC 0 (Python Compiler Prerequisites)
  ...
  └─ RB-0-0-1 (multi-file compilation) gates every epic that writes .flow code
...
| Missing stdlib function discovered late | Medium — blocks progress | Audit all Python stdlib usage (dict, list, str methods, pathlib, subprocess, tempfile) during EPIC 1 and add Flow equivalents before writing compiler code. |
...
| Performance — Flow compiler slower than Python | Low — only matters for bootstrap | Acceptable for bootstrap. Optimize after the compiler is self-hosted. |
