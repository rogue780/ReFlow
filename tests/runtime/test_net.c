/*
 * C-level tests for FL_Socket and net functions.
 *
...
static FL_Socket* _start_listener(fl_int* out_port) {
    FL_String* addr = fl_string_from_cstr("127.0.0.1");
    FL_Option_ptr opt = fl_net_listen(addr, 0);
    fl_string_release(addr);
    if (opt.tag == 0) return NULL;
    FL_Socket* listener = (FL_Socket*)opt.value;
...
static FL_Socket* _connect_to(fl_int port) {
    FL_String* host = fl_string_from_cstr("127.0.0.1");
    FL_Option_ptr opt = fl_net_connect(host, port);
    fl_string_release(host);
    if (opt.tag == 0) return NULL;
    return (FL_Socket*)opt.value;
}
...
static void test_listen_on_random_port(void) {
    ...
    fl_int port = 0;
    FL_Socket* listener = _start_listener(&port);
    assert(listener != NULL);
    assert(port > 0);

    fl_net_close(listener);
    PASS();
}
...
static void test_connect_and_accept(void) {
    ...
    FL_Socket* client = _connect_to(port);
    assert(client != NULL);

    FL_Option_ptr accept_opt = fl_net_accept(listener);
    assert(accept_opt.tag == 1);
    FL_Socket* server = (FL_Socket*)accept_opt.value;
    assert(server != NULL);
...
static void test_write_read_roundtrip(void) {
    ...
    fl_byte data[] = {0x48, 0x65, 0x6C, 0x6C, 0x6F}; /* "Hello" */
    FL_Array* send_arr = fl_array_new(5, sizeof(fl_byte), data);
    fl_bool wrote = fl_net_write(client, send_arr);
    assert(wrote == fl_true);
    fl_array_release(send_arr);

    /* Read bytes on server side */
    FL_Option_ptr read_opt = fl_net_read(server, 1024);
    assert(read_opt.tag == 1);
    FL_Array* recv_arr = (FL_Array*)read_opt.value;
    assert(fl_array_len(recv_arr) == 5);
    fl_byte* recv_data = (fl_byte*)recv_arr->data;
    assert(recv_data[0] == 0x48);
    assert(recv_data[1] == 0x65);
    assert(recv_data[2] == 0x6C);
    assert(recv_data[3] == 0x6C);
    assert(recv_data[4] == 0x6F);
    fl_array_release(recv_arr);
...
static void test_write_string_roundtrip(void) {
    ...
    FL_String* msg = fl_string_from_cstr("Hello, network!");
    fl_bool wrote = fl_net_write_string(client, msg);
    assert(wrote == fl_true);
    ...
    assert(read_opt.tag == 1);
    FL_Array* recv_arr = (FL_Array*)read_opt.value;
    assert(fl_array_len(recv_arr) == fl_string_len(msg));
    assert(memcmp(recv_arr->data, msg->data, (size_t)fl_string_len(msg)) == 0);
    fl_array_release(recv_arr);
...
static void test_read_after_close(void) {
    ...
    assert(read_opt.tag == 1);
    FL_Array* recv_arr = (FL_Array*)read_opt.value;
    assert(fl_array_len(recv_arr) == 0);
    fl_array_release(recv_arr);
...
static void test_remote_addr_format(void) {
    ...
    FL_Option_ptr addr_opt = fl_net_remote_addr(server);
    assert(addr_opt.tag == 1);
    FL_String* addr_str = (FL_String*)addr_opt.value;

    /* Should contain "127.0.0.1" */
    FL_String* expected_ip = fl_string_from_cstr("127.0.0.1");
    assert(fl_string_contains(addr_str, expected_ip) == fl_true);
    fl_string_release(expected_ip);
    fl_string_release(addr_str);
...
static void test_set_timeout(void) {
    ...
    fl_bool ok = fl_net_set_timeout(client, 5000);
    assert(ok == fl_true);

    fl_bool ok2 = fl_net_set_timeout(server, 1000);
    assert(ok2 == fl_true);
...
static void test_connect_to_nonexistent(void) {
    ...
    FL_String* host = fl_string_from_cstr("127.0.0.1");
    FL_Option_ptr opt = fl_net_connect(host, 1);
    fl_string_release(host);
...
static void test_multiple_writes_reads(void) {
    ...
    FL_String* msg1 = fl_string_from_cstr("AAA");
    FL_String* msg2 = fl_string_from_cstr("BBB");
    fl_net_write_string(client, msg1);
    fl_net_write_string(client, msg2);
    fl_string_release(msg1);
    fl_string_release(msg2);
    ...
    assert(read_opt.tag == 1);
    FL_Array* recv_arr = (FL_Array*)read_opt.value;
    assert(fl_array_len(recv_arr) == 6);
...
static void test_accept_null_listener(void) {
    ...
    FL_Option_ptr opt = fl_net_accept(NULL);
    assert(opt.tag == 0);
